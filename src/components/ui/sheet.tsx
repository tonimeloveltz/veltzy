...
const sheetVariants = cva(
  'fixed z-50 gap-4 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500',
  {
    variants: {
      ...
      // 'none' nao emite nenhuma utility bg-*. Serve para quem pinta o fundo por
      // classe de componente (ex: .sidebar-glass), que perderia para bg-background:
      // o layer utilities do Tailwind e emitido depois do layer components.
      surface: {
        default: 'bg-background',
        none: '',
      },
    },
    defaultVariants: { side: 'right', surface: 'default' },
  }
)
...
const SheetContent = React.forwardRef<
  ...
  SheetContentProps
>(({ side = 'right', surface = 'default', className, children, ...props }, ref) => (
  <SheetPortal>
    <SheetOverlay />
